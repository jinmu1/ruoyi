...
public class EIQClassifierTest {
    ...
    private List<String> testSortByDates;
    ...
    public void setUp() {
        ...
        testSortByDates =
                Arrays.asList("2024-02-02", "2024-02-03", "2024-02-04", "2024-02-05");
    ...
    public void getIKAnalysisTableTest(){
        List<IKAnalysisTable> result =  EIQClassifier.getIKAnalysisTable(testEIQBasicTable);
        // 保证结果行数跟输入一样
        ...
        assertEquals(
                ...
                result.stream()
                        .map(IKAnalysisTable::getCumulativeItemNumber)
                        .collect(Collectors.toList())
        );
        // 检查IK-物料号是对的
        assertEquals(
                testIKMaterialCode,
                result.stream()
                        .map(IKAnalysisTable::getMaterialCode)
                        .collect(Collectors.toList())
        );
        // 检查IK-物料名称是对的
        assertEquals(
                testMaterialName,
                result.stream()
                        .map(IKAnalysisTable::getMaterialName)
                        .collect(Collectors.toList())
        );
        ...
        assertEquals(
                testOccurrenceCount,
                result.stream()
                        .map(IKAnalysisTable::getOccurrenceCount)
                        .collect(Collectors.toList())
        );
