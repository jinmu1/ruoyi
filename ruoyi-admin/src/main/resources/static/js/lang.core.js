...
Array.prototype.avg = function(name) {
	...
	var avg = this.sum(name)/this.length;
    return avg;
};
...
//日期差计算
Date.sub = function(v1, v2, type){
	var d1 = Date.parseDate(v1);//开始日期
	var d2 = Date.parseDate(v2);//截止日期
	if( !d1 || !d2 ){
		return null;
	}
	var result = null;
...
var $List = Class.extend({
	...
	,
	sum: function(name, scale){
		return this.list.sum(name);
	}
});
...
(function(){
	...
	Debug.print = function(force){//打印；
		this.inst.print(force);
	};
...
var TaskQueue = Class.extend({
	...
	add : function( fun ){
		var _this = this, list = this.list;
		fun.state = 0;
		list.push(fun);
	},
	isStop : function(){//停止任务
		return this.isStop;
	},
	stop : function(){//停止任务
		var _this = this, list = this.list;
		if( _this.taskId ){//停止定时器
			clearInterval(_this.taskId);
		}
		$.each(list, function(i, fun){
			fun.stop = true;
		});
		_this.stop = true;
	},
	start : function(args){//开始任务
		var _this = this, list = this.list;
		_this.stop = false;
		var task = function(){
			$.each(list, function(i, fun){
				try{
					if( fun.state == 0 ){//未执行
						fun.state = 2;//执行中
						var result = fun(args);//返回 false 不继续执行
						fun.state = 1;
						if( result == false ){
							_this.stop = true;
							clearInterval(_this.taskId);
						...
						return false;
					}else if( fun.state == 1 ){//执行成功
						return true;//继续往下执行
					}else if( fun.state == 2 ){//执行中
						return false;//中断，等待现在任务继续执行
					}
				}catch(e){//出现异常，不继续执行
					console.log(e);
					fun.state = 3;//失败
		...
		_this.taskId = setInterval(task, 1000);
	}
});
